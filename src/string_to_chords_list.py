    ...
    while i < len(chord_string):
        ...
        if chord_string[i] == "(":
            ...
            while p < len(chord_notes):
                ...
                if chord_notes[p] not in notes_dict:
                    ...
                    if len(chord_notes) > p + 1 and chord_notes[p + 1] == "#":
                        list_of_pitches[note_number] = chord_notes[
                            index_of_note : index_of_note + 2
                        ]
                        p += 2
                    else:
                ...
                if len(chord_notes) > p and (
                    chord_notes[p] == "h" or chord_notes[p] == "l"
                ):
                    if chord_notes[p] == "h":
            ...
            while len(chord_string) > j and chord_string[j] != "(":
                if chord_string[j] != "-":
                    raise Exception(
                        "Invalid chord string. Ensure all notes are within parentheses."
                    )
                units += 1
                j += 1
            ...
            raise Exception(
                "Invalid chord string. Does your string not start with a '('?"
            )
